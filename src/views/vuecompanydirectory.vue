<script setup>
import MainSearch from './HomePage.vue'
import { ref } from 'vue'
const title = ref('')
</script>

<template>
  <div class = "wrapper">
  <input type ="text" placeholder="Search Staff Member" class="search"/>
  <span class="icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
</svg>
</span>
  </div>
<main>
  <MainSearch />
  
  </main>
</template>
<style scoped lang="postcss">

  .wrapper {
    @apply container mx-auto flex flex-col py-4 gap-8;
    .search{
      @apply px-3 py-10 placeholder-black text-slate-900 rounded-md border-0 outline-none focus:ring focus:ring-green-700;
    }
    .icon{
      @apply absolute right-0 z-10 py-8 pr-6 text-red-900 bg-black;
    }
  }

</style>
